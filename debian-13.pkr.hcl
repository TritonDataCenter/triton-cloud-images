/*
 * Debian 13 Packer template for building Triton DataCenter/SmartOS images
 */
...
locals {
  ...
  debian_13_boot_command = [
    "<wait><down>e<wait>",
    "<down><down><down><end>",
    "install <wait>",
    "url=${var.base_url}/debian-13.preseed.cfg ",
...
source "bhyve" "debian-13-x86_64" {
  boot_command       = local.debian_13_boot_command
  ...
  iso_checksum       = local.debian_13_iso_checksum
  iso_url            = local.debian_13_iso_url
  memory             = var.memory
  shutdown_command   = var.root_shutdown_command
  ssh_password       = var.ssh_password
  ssh_timeout        = var.ssh_timeout
  ssh_username       = var.ssh_username
  vm_name            = "debian-13-${formatdate("YYYYMMDD", timestamp())}.x86_64.zfs"
