...
locals {
  ...
  centos_stream_8_boot_command_uefi = [
    ...
    "inst.ks=${var.base_url}/centos-stream-8.ks<enter>",
    "initrdefi /images/pxeboot/initrd.img<enter>",
    "boot<enter><wait>"
  ]
}

source "bhyve" "centos-stream-8-x86_64" {
  boot_command       = local.centos_stream_8_boot_command_uefi
  ...
  iso_checksum       = local.centos_stream_8_iso_checksum
  iso_url            = local.centos_stream_8_iso_url
  memory             = var.memory
  shutdown_command   = var.root_shutdown_command
  ssh_password       = var.ssh_password
  ssh_timeout        = var.ssh_timeout
  ssh_username       = var.ssh_username
  vm_name            = "centos-stream-8-${formatdate("YYYYMMDD", timestamp())}.x86_64.zfs"
